...
class Net(nn.Module):
    def __init__(self, obs_dim, act_dim):
        ...
        self.outA = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=3, stride=3, padding=0),
            nn.ReLU(),
            nn.Conv2d(16, 4, kernel_size=1, stride=1, padding=0)
        )

        self.outV = nn.Sequential(
            nn.Conv2d(16, 16, kernel_size=3, stride=3, padding=0),
            nn.ReLU(),
    ...
    def forward(self, x):
        ...
        advantage = advantage.squeeze(2).squeeze(2)
        # print("advantage.shape is",advantage.shape)
        value = self.outV(x)
...
class DQN(object):
    def __init__(self, obs_dim, act_dim, args):
        ...
        self.eval_net= Net(obs_dim, act_dim).to(DEVICE)
    ...
    def save_model(self, episode):
        model_eval_path = self.path + "/dqn_eval_" + str(episode) + ".pth"
...
def main(args):
    ...
    while episode < args.max_episodes:
        ...
        # obs_new = obs_new[0]
        # print(obs_new)
        # obs_new = obs_new.transpose(1,2,0)
        # obs_new = obs_new.squeeze(1)
        # print(type(obs_new))
        # print(obs_new.shape)
        # print("obs_new[0] is", obs_new[0])
        # print("obs_new[1] is", obs_new[1])
        # print("obs_new[2] is", obs_new[2])
        # cv2.imwrite("observation.png", obs_new)
